                              ...
                              av_unused uint32_t       size)
{
    ...
    ap->segmentation =  (StdVideoAV1Segmentation) {

    };

    for (unsigned i = 0; i < STD_VIDEO_AV1_MAX_SEGMENTS; i++) {
        for (unsigned j = 0; j < STD_VIDEO_AV1_SEG_LVL_MAX; j++) {
    ...
    for (unsigned i = 0; i < STD_VIDEO_AV1_MAX_LOOP_FILTER_STRENGTHS; i++)
        ap->loop_filter.loop_filter_level[i] = frame_header->loop_filter_level[i];
    for (unsigned i = 0; i < STD_VIDEO_AV1_LOOP_FILTER_ADJUSTMENTS; i++)
    ...
    ap->global_motion = (StdVideoAV1GlobalMotion) {
